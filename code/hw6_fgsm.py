...
class Attacker:
    ...
    def attack(self, epsilon):
        ...
        for (data, target) in self.loader:
            ...
            # sec_index = 0
            # for i in range(len(output)):
            #     if output[i] > b and i != first_index:
            #         b = output[i]
            #         sec_index = i
            # c = 0
            # third_index = 0
            # for i in range(len(output)):
            #     if output[i] > b and i != first_index and i != sec_index:
            #         c = output[i]
            #         third_index = i
            # print(a, first_index)
            # print(b, sec_index)
        ...
        # final_acc = (fail / (wrong + success + fail))
            ...
            # count_string = str(pngcount%100) + str((pngcount%100)//10) + str(pngcount%10)
            count_string = "/{:03}.png".format(pngcount)
            pngcount += 1
            adv_ex = perturbed_data * torch.tensor(self.std, device = device).view(3, 1, 1) + torch.tensor(self.mean, device = device).view(3, 1, 1)
            adv_ex = adv_ex.squeeze().detach().cpu().numpy() 
            data_raw = data_raw * torch.tensor(self.std, device = device).view(3, 1, 1) + torch.tensor(self.mean, device = device).view(3, 1, 1)
            ...
            if epsilon == 0.1:
              ...
              plt.imsave(sys.argv[2] + count_string, adv_ex)
